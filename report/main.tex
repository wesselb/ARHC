...
\Cref{fig:comparison_priors} shows the effect of different settings of $\alpha_0$ and $\alpha_1$ on the performance of the adaptive compression scheme when the input is fed bit by bit. We see that for small $\alpha_0$ and $\alpha_1$ the compressor is able to adapt more quickly, but also has higher variance. This again reflects the classical convergence speed versus variance tradeoff. Furthermore, we observe that $\alpha_0=1.0$, $\alpha_1=1.0$ has initially a significant advantage. This shows that an appropriate prior can indeed help compression performance when little is known about the input.
